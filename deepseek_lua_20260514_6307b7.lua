-- ============================================================================
-- FEATURE 17: MODERN GUI (FULLY FIXED - MINIMIZE, FLOATING BAR, INFO TAB, STABLE)
-- ============================================================================

-- ============================================================================
-- GLOBAL VARIABLES UNTUK GUI (tetap menggunakan yang sudah ada)
-- ============================================================================
-- asumsi: screenGui, mainFrame, sidebar, contentPanel, floatingLogo, teleportButton, teleportButtonGui,
-- mainStroke, statusLabel, settingsContent, chatLog, chatInput, isLogoVisible, settingsContentCreated,
-- floatingBar, isFloatingVisible, infoContent sudah dideklarasikan di bagian atas script (sebagai nil)
-- Namun untuk keamanan, kita deklarasikan ulang dengan nilai default nil
floatingBar = nil
isFloatingVisible = false
infoContent = nil

-- Teks untuk menu Info (bisa diedit)
local infoText = [[
...
-- ============================================================================
-- DRAGGABLE (diperbaiki untuk mendukung semua input)
-- ============================================================================
...
-- ============================================================================
-- SETTINGS CONTENT (sama, dengan cleanup)
-- ============================================================================
local function createSettingsContent()
    if settingsContent and settingsContent.Parent then settingsContent:Destroy() end
    ...
    sendBtn.MouseButton1Click:Connect(function()
        ...
        updateScroll()
        chatListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateScroll)
        task.wait(2)
        newMsg:Destroy()
        updateScroll()
    end)
...
-- ============================================================================
-- INFO CONTENT (FIXED - scroll otomatis)
-- ============================================================================
local function createInfoContent()
    if infoContent and infoContent.Parent then infoContent:Destroy() end
    ...
    -- Tunggu render untuk mendapatkan ukuran sebenarnya
    task.wait()
    scrollFrame.CanvasSize = UDim2.new(0, 0, 0, textLabel.AbsoluteSize.Y + 10)
end

-- ============================================================================
-- FLOATING BAR (FIXED - menggunakan TextButton sebagai dasar)
-- ============================================================================
local function createFloatingBar()
    ...
    barButton.Name = "FloatingBarButton"
    ...
    textLabel.TextXAlignment = Enum.TextXAlignment.Left
    textLabel.Parent = barButton

    -- Drag untuk TextButton
...
-- ============================================================================
-- GRID BUTTON (sama, tidak berubah)
-- ============================================================================
...
-- ============================================================================
-- MAIN GUI (FIXED - navigation, minimize, floating)
-- ============================================================================
local function createGUI()
    ...
    -- Navigation handlers (cleanup yang aman)
    local function clearContent()
        if settingsContent and settingsContent.Parent then
            settingsContent:Destroy()
            settingsContent = nil
        end
        if infoContent and infoContent.Parent then
            infoContent:Destroy()
            infoContent = nil
        end
        if gridLayout.Parent ~= contentPanel then
            gridLayout.Parent = contentPanel
        end
    end
