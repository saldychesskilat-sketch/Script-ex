--[[
    ╔═══════════════════════════════════════════════════════════════════╗
    ║                    CYBERHEROES DELTA EXECUTOR v7.0               ║
    ...
    ║                     For Delta Executor                           ║
    ║   FIXED: Auto Win (optimized, no lag)                           ║
    ║   FIXED: Auto Task (smart teleport to nearest gen)              ║
    ║   FIXED: Auto Kill (teleport + multiple kill methods)           ║
    ║   NEW: Auto Generator + Skill Check Bypass (no more QTE!)       ║
    ║   NEW: Horizontal GUI (landscape layout)                        ║
    ║   NEW: All features default OFF                                 ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
-- ============================================================================
local config = {
    autoWinEnabled = false,          -- OFF by default
    autoTaskEnabled = false,         -- OFF by default
    taskRadius = 50,
    pathfindingParams = {
        AgentRadius = 2,
        AgentHeight = 5,
    ...
    espEnabled = false,              -- OFF by default
    highlightColorKiller = Color3.fromRGB(255, 50, 50),
    highlightColorSurvivor = Color3.fromRGB(50, 255, 50),
    highlightTransparency = 0.5,
    speedBoostEnabled = false,       -- OFF by default
    boostAmount = 20,
    boostDuration = 3,
    originalWalkSpeed = 16,
    stealthEnabled = false,          -- OFF by default
    stealthRadiusInvisible = 30,
    stealthRadiusVisible = 50,
    godModeEnabled = false,          -- OFF by default
    infiniteAmmoEnabled = false,     -- OFF by default
    shieldEnabled = false,           -- OFF by default
    shieldRadius = 30,
    tpwalkEnabled = false,           -- OFF by default
    tpwalkDuration = 2,
    tpwalkSlowSpeed = 0.5,
    noCollideEnabled = false,        -- OFF by default
    noCollideRadius = 30,
    massKillEnabled = false,         -- OFF by default
    massKillCooldown = 0,
    autoGeneratorEnabled = false,    -- OFF by default
    autoSkillCheckEnabled = false,   -- NEW: Auto Skill Check bypass
    guiVisible = true,
...
-- Teleport to generator position (most reliable)
local function teleportToGenerator(genPos)
    if not localRootPart then return false end
    pcall(function()
        localRootPart.CFrame = CFrame.new(genPos)
        print("[AutoTask] Teleported to generator position")
        return true
    end)
    return false
end

-- Auto task: teleport to nearest generator
...
-- Apply speed boost when damaged
local function applySpeedBoost()
    ...
    print("[SpeedBoost] Activated! Speed increased to " .. localHumanoid.WalkSpeed)
    ...
    if localHumanoid then
        ...
        print("[SpeedBoost] Deactivated. Speed restored to " .. config.originalWalkSpeed)
    end
...
local function startSpeedBoostMonitor()
    ...
    currentBoostConnection = RunService.Heartbeat:Connect(function()
        ...
        if lastHealth == 100 then
            lastHealth = localHumanoid.Health
        end
        
        -- Check if health decreased (damage taken)
        if localHumanoid.Health < lastHealth then
            print("[SpeedBoost] Damage detected! Health dropped from " .. lastHealth .. " to " .. localHumanoid.Health)
            applySpeedBoost()
        end
...
local function restartScript()
    ...
    -- Restart all systems (but all features are off)
    startAllSystems()
...
-- Apply slow movement (tpwalk) using killzone teleport method
local function applyTpwalk()
    ...
    -- Apply slow speed (this is the "walking slowly" effect)
    localHumanoid.WalkSpeed = config.tpwalkSlowSpeed
    isTpwalkActive = true
    print("[Tpwalk] Movement slowed for " .. config.tpwalkDuration .. " seconds")
    
    -- Also apply downward velocity to simulate being "dragged" or "slowed" (optional)
    if localRootPart then
        localRootPart.Velocity = Vector3.new(0, -10, 0)
    end
...
-- ============================================================================
-- FEATURE 13: AUTO GENERATOR + SKILL CHECK BYPASS (NO MORE QTE!)
-- ============================================================================

-- Find the remote event for generator repair
...
-- Force complete a generator by firing remote event
local function forceCompleteGenerator(generator)
    local remote = findGeneratorRemoteEvent()
    if remote then
        pcall(function()
            remote:FireServer(generator)
            print("[AutoGenerator] Force completed generator via remote event")
            return true
        end)
    end
    return false
...
-- Auto generator monitoring (lightweight)
local function startAutoGenerator()
    ...
    autoGeneratorConnection = RunService.Heartbeat:Connect(function()
        ...
        -- Scan for generators being repaired
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if isGenerator(obj) then
                -- Check if generator is being repaired (has progress but not complete)
                local progress = obj:FindFirstChild("Progress")
                if progress and (progress:IsA("NumberValue") or progress:IsA("IntValue")) then
                    if progress.Value > 0 and progress.Value < 100 then
                        forceCompleteGenerator(obj)
                    end
                end
            end
        end
    end)
    print("[AutoGenerator] Auto generator repair started (skips QTE minigame)")
end
...
local function stopAutoGenerator()
    ...
    print("[AutoGenerator] Auto generator repair stopped")
end

-- Skill Check Bypass (Auto Success)
local function startAutoSkillCheck()
    ...
    autoSkillCheckConnection = RunService.Heartbeat:Connect(function()
        ...
        -- Method 1: Look for skill check GUI elements and click them
                ...
                if gui:IsA("TextButton") or gui:IsA("ImageButton") then
                    ...
                    if name:find("skill") or name:find("check") or name:find("qte") or name:find("repair") then
                        if gui.Visible and gui.Active then
                            pcall(function()
                                gui:FireClick()
                                print("[AutoSkillCheck] Clicked skill check button")
                            end)
                        end
                    end
                end
        ...
        -- Method 2: Find and fire remote events related to skill checks
        for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
            if remote:IsA("RemoteEvent") then
                local name = remote.Name:lower()
                if name:find("skill") or name:find("check") or name:find("qte") or name:find("success") then
                    pcall(function()
                        remote:FireServer()
                        print("[AutoSkillCheck] Fired skill check remote event")
                    end)
                end
            end
        end
        
        -- Method 3: Use VirtualUser to simulate mouse click at center of screen
        if VirtualUser then
            pcall(function()
                local viewportSize = workspace.CurrentCamera.ViewportSize
                VirtualUser:ClickButton1(Vector2.new(viewportSize.X / 2, viewportSize.Y / 2))
            end)
        end
    end)
...
local function createGUI()
    ...
    -- Daftar tombol (7 tombol per baris) - SEMUA MATI (OFF)
    local row1Buttons = {
    ...
    task.spawn(function()
        while screenGui and screenGui.Parent do
            local activeCount = (config.autoWinEnabled and 1 or 0) + 
                                ...
                                (config.autoGeneratorEnabled and 1 or 0) +
                                (config.autoSkillCheckEnabled and 1 or 0)
...
-- ============================================================================
-- AUTO WIN & AUTO TASK & AUTO GENERATOR COMBINED METHODS (BRUTE FORCE)
-- ============================================================================

-- List of multiple methods for Auto Win
local autoWinMethods = {
    function() -- Method 1: RemoteEvent FireServer
        local remote = findRepairRemoteEvent()
        if remote then pcall(function() remote:FireServer() end) end
    end,
    function() -- Method 2: ClickDetector FireClick
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ClickDetector") and obj.Enabled then
                pcall(function() obj:FireClick() end)
            end
        end
    end,
    function() -- Method 3: ProximityPrompt Hold
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled then
                pcall(function() obj:Hold(); task.wait(0.1); obj:Release() end)
            end
        end
    end,
    function() -- Method 4: Direct Value Manipulation (Progress & Completed)
        for _, obj in ipairs(Workspace:GetDescendants()) do
            local progress = obj:FindFirstChild("Progress")
            local completed = obj:FindFirstChild("Completed")
            if progress then pcall(function() progress.Value = 100 end) end
            if completed then pcall(function() completed.Value = true end) end
        end
    end,
    function() -- Method 5: Fire All RemoteEvents in ReplicatedStorage
    ...
    function() -- Method 6: Fire All RemoteFunctions
        for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
    ...
    function() -- Method 7: BreakJoints on generators
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if isGenerator(obj) then
    ...
    function() -- Method 8: Set Progress IntValue
        for _, obj in ipairs(Workspace:GetDescendants()) do
            local progress = obj:FindFirstChild("Progress")
    ...
    function() -- Method 9: Set Progress NumberValue
        for _, obj in ipairs(Workspace:GetDescendants()) do
            local progress = obj:FindFirstChild("Progress")
    ...
    function() -- Method 10: Trigger TouchInterest
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("BasePart") and obj.Name:lower():find("gen") then
    ...
    function() -- Method 11: Call 'repair' function in LocalScript
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("LocalScript") and obj.Name:lower():find("repair") then
                pcall(function() obj:Clone() end)
            end
        end
    end,
    function() -- Method 12: Call 'gen' function in ModuleScript
        for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
            if obj:IsA("ModuleScript") and (obj.Name:lower():find("gen") or obj.Name:lower():find("repair")) then
                pcall(function() require(obj)() end)
            end
        end
    end,
    function() -- Method 13: Call 'repair' remote event with specific arguments
        local remote = findRepairRemoteEvent()
        if remote then
            pcall(function() remote:FireServer("Complete") end)
            pcall(function() remote:FireServer(1) end)
            pcall(function() remote:FireServer(true) end)
        end
    end,
    function() -- Method 14: Fire all remote events in workspace
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("RemoteEvent") then
                pcall(function() obj:FireServer() end)
            end
    ...
    function() -- Method 15: Find and call repair function in tool
        for _, tool in ipairs(localPlayer.Backpack:GetChildren()) do
            if tool:IsA("Tool") and tool.Name:lower():find("repair") then
                pcall(function() tool:Activate() end)
            end
        end
    end,
    function() -- Method 16: Force humanoid to sit (sometimes completes generators)
        if localHumanoid then
            pcall(function() localHumanoid.Sit = true end)
            task.wait(0.1)
            pcall(function() localHumanoid.Sit = false end)
        end
    end,
    function() -- Method 17: Click all parts named "Button" or "Activate"
        for _, obj in ipairs(Workspace:GetDescendants()) do
    ...
    function() -- Method 18: Fire all remote events in Players
    ...
    function() -- Method 19: Use VirtualUser to simulate key press
        if VirtualUser then
            pcall(function() VirtualUser:ClickButton1(Vector2.new(500, 500)) end)
        end
    end,
    function() -- Method 20: Set 'repaired' value
        for _, obj in ipairs(Workspace:GetDescendants()) do
            local repaired = obj:FindFirstChild("Repaired")
            if repaired and repaired:IsA("BoolValue") then
                pcall(function() repaired.Value = true end)
            end
        end
    end
}

-- List of multiple methods for Auto Task
local autoTaskMethods = {
    function() -- Method 1: Teleport to generator position
        local nearest = getNearestGeneratorOptimized()
        if nearest and localRootPart then
            local pos = nearest:GetPivot().Position
            pcall(function() localRootPart.CFrame = CFrame.new(pos) end)
        end
    end,
    function() -- Method 2: Move to generator using Humanoid:MoveTo
        local nearest = getNearestGeneratorOptimized()
        if nearest and localHumanoid then
            local pos = nearest:GetPivot().Position
            pcall(function() localHumanoid:MoveTo(pos) end)
        end
    end,
    function() -- Method 3: Use TweenService to move smoothly
        local nearest = getNearestGeneratorOptimized()
        if nearest and localRootPart then
            local pos = nearest:GetPivot().Position
            local tween = TweenService:Create(localRootPart, TweenInfo.new(1), {CFrame = CFrame.new(pos)})
            tween:Play()
            tween.Completed:Wait()
        end
    end,
    function() -- Method 4: Pathfinding to generator
        local nearest = getNearestGeneratorOptimized()
        if nearest then
            local targetPart = nearest:FindFirstChildWhichIsA("BasePart") or nearest
            moveWithPathfinding(targetPart)
        end
    end,
    function() -- Method 5: Teleport to random generator
        local generators = {}
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if isGenerator(obj) then
        ...
        if #generators > 0 and localRootPart then
            local randomGen = generators[math.random(1, #generators)]
            local pos = randomGen:GetPivot().Position
            pcall(function() localRootPart.CFrame = CFrame.new(pos) end)
        end
    end,
    function() -- Method 6: Move to generator using BodyVelocity
        local nearest = getNearestGeneratorOptimized()
        if nearest and localRootPart then
            local pos = nearest:GetPivot().Position
            local direction = (pos - localRootPart.Position).Unit
    ...
    function() -- Method 7: Set Humanoid.WalkSpeed to max and move
        if localHumanoid then
    ...
    function() -- Method 8: Use ClickToMove (if available)
        local nearest = getNearestGeneratorOptimized()
        if nearest and localHumanoid then
            local pos = nearest:GetPivot().Position
            pcall(function() localHumanoid:MoveTo(pos) end)
    ...
    function() -- Method 9: Teleport to generator and set CFrame
        local nearest = getNearestGeneratorOptimized()
        if nearest and localRootPart then
            local pos = nearest:GetPivot().Position
            pcall(function() localRootPart.CFrame = CFrame.new(pos + Vector3.new(0, 5, 0)) end)
        end
    end,
    function() -- Method 10: Move to generator using TweenService and BodyPosition
        local nearest = getNearestGeneratorOptimized()
        if nearest and localRootPart then
            local pos = nearest:GetPivot().Position
...
-- List of multiple methods for Auto Generator (skip QTE)
local autoGeneratorMethods = {
    function() -- Method 1: Fire repair remote event
        local remote = findGeneratorRemoteEvent()
        if remote then pcall(function() remote:FireServer() end) end
    end,
    function() -- Method 2: Set progress to 100
        for _, obj in ipairs(Workspace:GetDescendants()) do
            local progress = obj:FindFirstChild("Progress")
            if progress then pcall(function() progress.Value = 100 end) end
        end
    end,
    function() -- Method 3: Fire ClickDetector on generator
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ClickDetector") and obj.Enabled then
                pcall(function() obj:FireClick() end)
            end
    ...
    function() -- Method 4: Hold ProximityPrompt on generator
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled then
                pcall(function() obj:Hold(); task.wait(0.1); obj:Release() end)
            end
    ...
    function() -- Method 5: Set completed to true
        for _, obj in ipairs(Workspace:GetDescendants()) do
            local completed = obj:FindFirstChild("Completed")
            if completed then pcall(function() completed.Value = true end) end
        end
    end,
    function() -- Method 6: Fire all repair-related remote events
        for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
            if obj:IsA("RemoteEvent") and obj.Name:lower():find("repair") then
                pcall(function() obj:FireServer() end)
            end
        end
    end,
    function() -- Method 7: Call require on repair module
        for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
            if obj:IsA("ModuleScript") and obj.Name:lower():find("repair") then
                pcall(function() require(obj)() end)
            end
        end
    end,
    function() -- Method 8: Fire remote event with generator object
        local remote = findGeneratorRemoteEvent()
        if remote then
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if isGenerator(obj) then
    ...
    function() -- Method 9: Fire all remote events in workspace
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("RemoteEvent") then
                pcall(function() obj:FireServer() end)
            end
    ...
    function() -- Method 10: Use VirtualUser to simulate repair input
        if VirtualUser then
            pcall(function() VirtualUser:ClickButton1(Vector2.new(500, 500)) end)
        end
    end
...
-- Combined brute force execution for Auto Win
local function bruteForceAutoWin()
    if not config.autoWinEnabled then return end
    for _, method in ipairs(autoWinMethods) do
        pcall(method)
    end
end

-- Combined brute force execution for Auto Task
local function bruteForceAutoTask()
    if not config.autoTaskEnabled then return end
    for _, method in ipairs(autoTaskMethods) do
        pcall(method)
    end
end

-- Combined brute force execution for Auto Generator
local function bruteForceAutoGenerator()
    if not config.autoGeneratorEnabled then return end
    for _, method in ipairs(autoGeneratorMethods) do
        pcall(method)
    end
end

-- Combined brute force loop
local function startBruteForceLoop()
    if bruteForceConnection then return end
    bruteForceConnection = RunService.Heartbeat:Connect(function()
        bruteForceAutoWin()
        bruteForceAutoTask()
        bruteForceAutoGenerator()
    end)
    print("[BruteForce] Combined brute force loop started (20+ methods)")
end

local function stopBruteForceLoop()
    if bruteForceConnection then
        bruteForceConnection:Disconnect()
        bruteForceConnection = nil
    end
    print("[BruteForce] Combined brute force loop stopped")
end
...
-- Start all systems (but all features are off by default)
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║                    CYBERHEROES DELTA EXECUTOR v7.0               ║")
