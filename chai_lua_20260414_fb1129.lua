--[[
    ...
    ║            MASS TELEPORT & AUTO INTERACT (FIXED!)                ║
    ║            GENERATOR ESP + AUTO REPAIR (WITHOUT QTE)             ║
    ║            AUTO AIM (KILLER LOCK) + TELEPORT TO SURVIVOR         ║
    ║              Survivor vs Killer - Generator Fixer                ║
    ║                   Developed by Deepseek-CH                       ║
    ║                     For Delta Executor                           ║
    ║   FIXED: Mass Kill now teleports & presses E automatically      ║
    ║   FIXED: Auto Generator with ESP & auto-repair                   ║
    ║   FIXED: Persistent GUI (CoreGui + ResetOnSpawn = false)        ║
    ║   FIXED: Separate Logo Button & Teleport Button                 ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
-- Fungsi untuk mensimulasikan tekanan tombol E
...
-- ============================================================================
-- FEATURE 3: ESP SYSTEM (PLAYER + GENERATOR)
-- ============================================================================
...
local function updateGeneratorESP()
    if not config.espEnabled then
        for _, data in pairs(generatorEspConnections) do
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if isGenerator(obj) and not generatorEspConnections[obj] then
            local highlight = Instance.new("Highlight")
            ...
            generatorEspConnections[obj] = highlight
        end
    end
end
...
-- ============================================================================
-- FEATURE 12: MASS TELEPORT & AUTO INTERACT (FIXED!)
-- ============================================================================
...
local function massTeleportAndKill()
    ...
    if isMassKilling then 
        print("[MassKill] Already in progress...")
        return 
    end
    ...
    for _, survivor in ipairs(survivors) do
        if survivor and survivor.Character then
            if teleportBehindPlayer(survivor) then
                task.wait(0.05)
                pressE()  -- Simulate pressing E to interact
...
-- ============================================================================
-- FEATURE 13: AUTO GENERATOR & AUTO REPAIR (WITHOUT QTE)
-- ============================================================================
...
local function startAutoGenerator()
    ...
    print("[AutoGenerator] Auto generator repair started (bypasses QTE)")
end
...
-- ============================================================================
-- FEATURE 15: TELEPORT TO NEAREST SURVIVOR
-- ============================================================================
...
-- ============================================================================
-- FEATURE 16: PERSISTENT GUI & SEPARATE BUTTONS
-- ============================================================================

-- Logo floating (kotak kecil, RGB, saat ditekan membuka menu utama)
...
local function init()
    ...
    print("║                 + MASS TELEPORT & AUTO INTERACT (FIXED!)          ║")
    print("║                 + AUTO GENERATOR + GENERATOR ESP                 ║")
    print("║                 + AUTO AIM + TELEPORT TO SURVIVOR                ║")
