--[[
    ...
    ║                     For Delta Executor                           ║
    ║   FIXED: Mass Kill random target, teleport behind, lock camera  ║
    ║   FIXED: GUI floating logo reappears when closed                ║
    ║   NEW: Settings menu with color picker & fake chat report       ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
local config = {
    ...
    lastHealth = 100,
    guiThemeColor = Color3.fromRGB(0, 230, 255)  -- default cyan
}
...
-- Teleport ke belakang target dengan jarak 2 studs
...
local function createSettingsContent()
    ...
    -- Color picker sederhana (3 pilihan warna)
...
local function createGUI()
    ...
    aboutItem.MouseButton1Click:Connect(function()
        ...
        if settingsContent then settingsContent:Destroy() end
        gridLayout.Parent = contentPanel
        -- Could add about content, but not required now
    end)
...
local function init()
    ...
    print("║                 + MASS KILL LOOP (RANDOM + LOCK CAMERA)           ║")
