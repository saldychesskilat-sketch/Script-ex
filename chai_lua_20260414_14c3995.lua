--[[
    ...
    ║                     For Delta Executor                           ║
    ║   NEW: Mass Kill Loop (continuous teleport + press E)           ║
    ║   NEW: Auto Generator with ESP & teleport                       ║
    ║   FIXED: GUI permanen di CoreGui (tahan cutscene)               ║
    ║   FIXED: Logo bulat untuk membuka menu, terpisah dari tombol TP ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
local config = {
    ...
    noCollideRadius = 30,
    massKillEnabled = false,          -- MASS KILL LOOP (NEW)
    autoGeneratorEnabled = false,      -- AUTO GENERATOR (NEW)
...
local massKillLoopConnection = nil       -- Loop untuk mass kill
local autoGeneratorLoopConnection = nil  -- Loop untuk auto generator
...
local generatorEspHighlights = {}        -- ESP untuk generator
...
-- Simulasi tekan tombol E (interaksi) menggunakan VirtualInputManager
local function simulatePressE()
    -- Kirim event key press untuk tombol E
    pcall(function()
        VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)
        task.wait(0.05)
        VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.E, false, game)
    end)
    -- Alternatif: gunakan VirtualUser jika tersedia
    pcall(function()
...
-- ============================================================================
-- FEATURE 12: MASS KILL LOOP (TELEPORT TO SURVIVOR + PRESS E)
-- ============================================================================
local function getNearestSurvivorForMassKill()
    local nearest = nil
...
local function massKillLoop()
    ...
    local target = getNearestSurvivorForMassKill()
...
local function startMassKillLoop()
    ...
    print("[MassKill] Mass kill loop started (teleport + press E)")
end
...
-- Auto generator loop: cari generator terdekat, teleport, tekan E
local function autoGeneratorLoop()
    ...
    if nearestGen then
        ...
        teleportTo(genPos)
        task.wait(0.1)
        simulatePressE()  -- Mulai perbaikan
        print("[AutoGenerator] Started repair on generator")
    end
    task.wait(1.0) -- Cek setiap detik
end
...
local function startAutoGeneratorLoop()
    ...
    -- Juga update ESP setiap beberapa detik
    task.spawn(function()
    ...
    print("[AutoGenerator] Auto generator started (teleport + press E + ESP)")
end
...
local function startAutoSkillCheck()
    ...
    autoSkillCheckConnection = RunService.Heartbeat:Connect(function()
        ...
        if not getLocalCharacter() then return end
        -- Generic method: find skill check GUI
        local playerGui = localPlayer:FindFirstChild("PlayerGui")
        if playerGui then
...
-- ============================================================================
-- FEATURE 16: TELEPORT TO NEAREST SURVIVOR (BUKAN KILLER)
-- ============================================================================
...
-- ============================================================================
-- FEATURE 17: MODERN GUI (PERMANEN DI COREGUI, LOGO BULAT, TOMBOL TP TERPISAH)
-- ============================================================================

-- Logo floating (bulat kecil, RGB) untuk membuka menu utama
local function createFloatingLogo()
    ...
    floatingLogo.Size = UDim2.new(0, 35, 0, 35)   -- ukuran bulat
    ...
    corner.CornerRadius = UDim.new(1, 0)   -- bulat penuh
...
-- Tombol teleport permanen (bulat, terpisah) - teleport ke survivor terdekat
local function createPermanentTeleportButton()
    ...
    btnCorner.CornerRadius = UDim.new(1, 0)   -- bulat
...
-- Main GUI dengan ukuran 2x lebih kecil (di CoreGui agar tahan cutscene)
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║                    CYBERHEROES DELTA EXECUTOR v9.0               ║")
