--[[
    CYBERHEROES DELTA EXECUTOR v10.2 - UPGRADED
    ============================================
    Upgrade focus:
    - Auto Generator: faster, stable, great skill check (adapted from Violence District)
    - ESP: distance culling, update rate, max objects, priority nearby
    - Mass Kill: only when Killer, smart target selection, radius control
    - Killer Power Trigger: instant, responsive
    - Instant Complete Generator: one-click full completion
    All original features & UI preserved. Mobile compatible.
--]]
...
if not _G.CyberHeroesState then
    _G.CyberHeroesState = {
        config = {
            ...
            massKillRadius = 40,           -- NEW: radius for mass kill
            ...
            espUpdateRate = 0.5,
            espMaxObjects = 100,
            espUseDistanceCulling = true,
            -- NEW generator settings
            generatorMode = "great",        -- "great" or "normal"
            -- NEW mass kill settings
            massKillDelay = 0.15,
        },
        featuresActive = {}
    }
...
local espObjects = {} -- for caching

-- NEW generator remotes cache
local cachedRepairEvent = nil
local cachedSkillCheckEvent = nil
...
local function startAutoWin()
    ...
    print("[AutoWin] Auto win started")
end
...
-- ============================================================================
-- FEATURE 2: AUTO TASK (unchanged, but keep as is)
-- ============================================================================
...
local function autoTaskLoop()
    ...
    if isPlayerHooked() then
        ...
        if knockbackKiller(killerChar) then
            print("[AutoTask] Knocked back killer")
            activateAuto1xMode()
        end
        task.wait(0.5)
        return
    ...
    if leverGoal then
        ...
        if gateFO then interactWithGate(gateFO) end
        local rightGate = findRightGate()
        if rightGate then interactWithGate(rightGate) end
        local liftGate = findLiftGate()
        if liftGate then interactWithGate(liftGate) end
    else
...
-- ============================================================================
-- FEATURE 3: ESP SYSTEM (ENHANCED with distance culling, update rate, max objects)
-- ============================================================================
local function getDistanceFromPlayer(pos)
    if not localRootPart then return math.huge end
    return (pos - localRootPart.Position).Magnitude
end
...
local function createHighlightForPlayer(player)
    ...
    if root and config.espUseDistanceCulling then
        local dist = getDistanceFromPlayer(root.Position)
...
local function updateAllESP()
    ...
    if currentTime - lastESPUpdate < config.espUpdateRate then return end
    ...
    for _, player in ipairs(players) do
        if player ~= localPlayer then
            if count >= config.espMaxObjects then break end
...
local function startESP()
    ...
    espUpdateConnection = RunService.Heartbeat:Connect(updateAllESP)
    ...
    updateAllESP()
    print("[ESP] Enhanced ESP started")
end
...
-- ============================================================================
-- FEATURE 4: SPEED BOOST (unchanged)
-- ============================================================================
...
local function startInfiniteAmmo()
    ...
    infiniteAmmoConnection = RunService.Heartbeat:Connect(function()
        ...
        if backpack then for _, tool in ipairs(backpack:GetChildren()) do if tool:IsA("Tool") then setAmmo(tool) end end end
        if character then for _, tool in ipairs(character:GetChildren()) do if tool:IsA("Tool") then setAmmo(tool) end end end
    end)
end
...
-- ============================================================================
-- FEATURE 8: SCRIPT RESTART (unchanged but updated with new features)
-- ============================================================================
...
local function checkTpwalkProximity()
    ...
    if nearestKillerDistance <= 30 then
        if not isTpwalkActive then applyTpwalkBoost() end
    end
end
...
-- ============================================================================
-- FEATURE 11: NO COLLIDE (unchanged)
-- ============================================================================
...
-- ============================================================================
-- FEATURE 12: MASS KILL (ENHANCED: only killer, smart radius, target selection)
-- ============================================================================
local function isLocalPlayerKiller()
    if not localPlayer.Team then return false end
    local teamName = localPlayer.Team.Name:lower()
    return teamName:find("killer") or teamName:find("monster") or teamName:find("enemy")
end

local function getNearestSurvivorWithinRadius(radius)
    if not localRootPart then return nil, nil end
    local localPos = localRootPart.Position
    local nearest = nil
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer then
            ...
            if char then
                ...
                if player.Team then
                    isSurvivor = (player.Team.Name:lower():find("survivor") or player.Team.Name:lower():find("survivors"))
                end
...
local function massKillLoop()
    ...
    if not isLocalPlayerKiller() then return end
    if not getLocalCharacter() or not localRootPart then return end

    local target, dist = getNearestSurvivorWithinRadius(config.massKillRadius)
    if target and target.Character then
        ...
        if targetRoot then
            ...
            if hitSurvivorWithHilt(target) then
                print("[MassKill] Killed " .. target.Name)
            else
                simulatePressE()
    ...
    task.wait(config.massKillDelay)
end
...
local function startMassKillLoop()
    ...
    print("[MassKill] Enhanced mass kill started (killer only, radius " .. config.massKillRadius .. ")")
end
...
-- ============================================================================
-- FEATURE 13: AUTO GENERATOR (ENHANCED: faster, stable, great skill check)
-- ============================================================================
local function getGeneratorRemotes()
    if cachedRepairEvent and cachedSkillCheckEvent then return cachedRepairEvent, cachedSkillCheckEvent end
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    if remotes then
        local genRemotes = remotes:FindFirstChild("Generator")
        if genRemotes then
            cachedRepairEvent = genRemotes:FindFirstChild("RepairEvent")
            cachedSkillCheckEvent = genRemotes:FindFirstChild("SkillCheckResultEvent")
        end
    end
    return cachedRepairEvent, cachedSkillCheckEvent
end
...
local function repairGenerator(genObj)
    if not genObj then return false end
    local repairEvent, skillCheckEvent = getGeneratorRemotes()
    if not repairEvent or not skillCheckEvent then return false end
    
    -- Find generator point
    local genPoint = nil
    for _, child in ipairs(genObj:GetChildren()) do
        if child.Name:find("GeneratorPoint") then
            genPoint = child
            break
        end
    end
    if not genPoint then return false end
    
    safeCall(function()
        repairEvent:FireServer(genPoint, true)
        if config.generatorMode == "great" then
            skillCheckEvent:FireServer("success", 1, genObj, genPoint)
        else
            skillCheckEvent:FireServer("neutral", 0, genObj, genPoint)
        end
    end)
    return true
end
...
local function autoGeneratorLoop()
    ...
    if nearestGen then
        ...
        teleportTo(genPos)
        task.wait(0.1)
        repairGenerator(nearestGen)
        print("[AutoGenerator] Repaired generator (great mode)")
    end
    task.wait(0.5) -- slightly faster than before
end
...
local function startAutoGeneratorLoop()
    ...
    print("[AutoGenerator] Enhanced auto generator started (great skill check)")
end
...
local function teleportToNearestSurvivor()
    ...
    if targetPlayer and targetPlayer.Character then
        ...
        print("[Teleport] No survivor found")
    end
end

-- ============================================================================
-- NEW FEATURE: INSTANT COMPLETE ALL GENERATORS (one-click)
-- ============================================================================
local function instantCompleteAllGenerators()
    local repairEvent, skillCheckEvent = getGeneratorRemotes()
    if not repairEvent or not skillCheckEvent then
        print("[InstantGen] Remote events not found")
        return false
    end
    
    local map = Workspace:FindFirstChild("Map")
    if not map then
        print("[InstantGen] Map not found")
        return false
    end
    ...
    print("[InstantGen] Completed " .. completed .. " generator points")
    return completed > 0
end

-- ============================================================================
-- NEW FEATURE: KILLER POWER TRIGGER (instant)
-- ============================================================================
local function triggerKillerPower()
    if not isLocalPlayerKiller() then
        print("[KillerPower] Not a killer")
        return false
    end
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    if remotes then
        ...
        if killerRemotes then
            ...
            if killerFolder then
                ...
                if activatePower then
                    safeCall(function() activatePower:FireServer() end)
                    print("[KillerPower] Activated")
                    return true
                end
            end
        end
    end
    print("[KillerPower] No remote found")
    return false
end

-- ============================================================================
-- FEATURE 17: MODERN GUI (enhanced with new buttons)
-- ============================================================================
...
local function createGridButton(parent, name, text, initialState, onChange)
    ...
    button.MouseButton1Click:Connect(function()
        if name == "restartScript" then
            restartScript()
            return
        end
        if name == "instantCompleteGen" then
            instantCompleteAllGenerators()
            return
        end
        if name == "triggerKillerPower" then
            triggerKillerPower()
            return
        end
...
local function createGUI()
    ...
    gridLayout.CellSize = UDim2.new(0, 85, 0, 32)
    ...
    local features = {
        ...
        {name="instantCompleteGen", text="INSTANT GEN"},   -- NEW
        {name="triggerKillerPower", text="KILLER POWER"},  -- NEW
        {name="restartScript", text="RESTART"}
    }
    for _, feat in ipairs(features) do
        local initialState = (feat.name ~= "restartScript" and feat.name ~= "instantCompleteGen" and feat.name ~= "triggerKillerPower") and config[feat.name] or false
...
-- ============================================================================
-- RESTORE FEATURE STATES FUNCTION (updated)
-- ============================================================================
local function restoreFeatureStates()
    print("[State] Restoring feature states...")
    
    if config.autoWinEnabled and not autoWinConnection then startAutoWin() elseif not config.autoWinEnabled and autoWinConnection then stopAutoWin() end
    if config.autoTaskEnabled and not currentTaskConnection then startAutoTask() elseif not config.autoTaskEnabled and currentTaskConnection then stopAutoTask() end
    if config.speedBoostEnabled and not currentBoostConnection then startSpeedBoostMonitor() elseif not config.speedBoostEnabled and currentBoostConnection then stopSpeedBoostMonitor() end
    if config.stealthEnabled and not stealthConnection then startStealthMonitor() elseif not config.stealthEnabled and stealthConnection then stopStealthMonitor() end
    if config.godModeEnabled and not godModeConnection then startGodMode() elseif not config.godModeEnabled and godModeConnection then stopGodMode() end
    if config.infiniteAmmoEnabled and not infiniteAmmoConnection then startInfiniteAmmo() elseif not config.infiniteAmmoEnabled and infiniteAmmoConnection then stopInfiniteAmmo() end
    if config.shieldEnabled and not shieldConnection then startShieldMonitor() elseif not config.shieldEnabled and shieldConnection then stopShieldMonitor() end
    if config.tpwalkEnabled and not tpwalkConnection then startTpwalkMonitor() elseif not config.tpwalkEnabled and tpwalkConnection then stopTpwalkMonitor() end
    if config.noCollideEnabled and not noCollideConnection then startNoCollideMonitor() elseif not config.noCollideEnabled and noCollideConnection then stopNoCollideMonitor() end
    if config.massKillEnabled and not massKillLoopConnection then startMassKillLoop() elseif not config.massKillEnabled and massKillLoopConnection then stopMassKillLoop() end
    if config.autoGeneratorEnabled and not autoGeneratorLoopConnection then startAutoGeneratorLoop() elseif not config.autoGeneratorEnabled and autoGeneratorLoopConnection then stopAutoGeneratorLoop() end
    if config.autoSkillCheckEnabled and not autoSkillCheckConnection then startAutoSkillCheck() elseif not config.autoSkillCheckEnabled and autoSkillCheckConnection then stopAutoSkillCheck() end
    if config.autoAimEnabled and not autoAimConnection then startAutoAim() elseif not config.autoAimEnabled and autoAimConnection then stopAutoAim() end
...
-- ============================================================================
-- AUTO RECOVERY SYSTEM (unchanged)
-- ============================================================================
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║         CYBERHEROES DELTA EXECUTOR v10.2 - UPGRADED              ║")
    print("║   Enhanced: Auto Generator (great skill), ESP (distance cull)    ║")
    print("║   Enhanced: Mass Kill (killer only, smart radius)                ║")
    print("║   NEW: Instant Complete Generator, Killer Power Trigger          ║")
