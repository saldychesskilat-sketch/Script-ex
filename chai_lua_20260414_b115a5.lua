--[[
    ╔═══════════════════════════════════════════════════════════════════╗
    ║                    CYBERHEROES DELTA EXECUTOR v8.3               ║
    ...
    ║                     For Delta Executor                           ║
    ║   FIXED: Logo kotak kecil, teleport hanya ke survivor           ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
-- Tombol teleport permanen (terpisah)
...
local function createHighlightForPlayer(player)
    ...
    local isKillerFlag = isKiller(player)
    local highlightColor = isKillerFlag and config.highlightColorKiller or config.highlightColorSurvivor
...
-- ============================================================================
-- FEATURE 15: TELEPORT TO NEAREST SURVIVOR (BUTTON) - HANYA KE SURVIVOR
-- ============================================================================
local function teleportToNearestSurvivor()
    ...
    if nearest and nearest.Character then
        ...
        if targetRoot then
            ...
            print("[Teleport] Teleported to survivor: " .. nearest.Name)
        end
    else
        print("[Teleport] No nearby survivor found")
    end
end

-- ============================================================================
-- FEATURE 16: MODERN GUI (UKURAN KECIL, TAHAN CUTSCENE, LOGO KOTAK)
-- ============================================================================
-- Tempatkan GUI di CoreGui agar tidak hilang saat cutscene

-- LOGO KOTAK KECIL dengan sudut membulat (bukan bulat)
local function createFloatingLogo()
    ...
    floatingLogo.Size = UDim2.new(0, 40, 0, 40)  -- ukuran kecil kotak
    ...
    -- Sudut kotak (rounded square)
    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 8)  -- membulat, bukan lingkaran penuh
...
-- Tombol teleport permanen (tetap di PlayerGui) - sekarang teleport ke survivor
local function createPermanentTeleportButton()
    ...
    teleportButtonGui.Parent = localPlayer:FindFirstChild("PlayerGui") or localPlayer.PlayerGui
    teleportButton = Instance.new("TextButton")
    teleportButton.Name = "TeleportButton"
    teleportButton.Size = UDim2.new(0, 60, 0, 60)
    ...
    btnCorner.CornerRadius = UDim.new(0, 8)  -- sudut kotak
...
-- Toggle button untuk fitur (grid item) dengan ukuran lebih kecil
local function createGridButton(parent, name, text, initialState, onChange)
    local button = Instance.new("TextButton")
    button.Name = name
    button.Size = UDim2.new(0, 85, 0, 32)  -- diperkecil
...
local function createSidebarItem(parent, text, icon, active)
    local button = Instance.new("TextButton")
    button.Size = UDim2.new(1, 0, 0, 28)  -- lebih kecil
...
-- Main GUI dengan ukuran 2x lebih kecil
local function createGUI()
    if screenGui then screenGui:Destroy() end
    screenGui = Instance.new("ScreenGui")
    screenGui.Name = "CyberHeroes_GUI"
    screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    screenGui.Parent = CoreGui  -- Tempatkan di CoreGui agar tidak hilang saat cutscene
    screenGui.ResetOnSpawn = false

    -- Main window ukuran 360x240 (setengah dari 720x480)
    ...
    -- Fungsi untuk menyembunyikan GUI dan menampilkan logo kotak
    ...
    -- Sidebar (kiri) dengan lebar 80px
    ...
    -- Panel kanan (grid 4 kolom) ukuran disesuaikan
    ...
    for _, feat in ipairs(features) do
        local initialState = (feat.name ~= "executeMassKill" and feat.name ~= "restartScript") and config[feat.name] or false
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║                    CYBERHEROES DELTA EXECUTOR v8.3               ║")
    ...
    print("║                 + AUTO AIM + TELEPORT TO NEAREST SURVIVOR        ║")
