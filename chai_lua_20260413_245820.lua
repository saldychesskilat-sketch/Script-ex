--[[
    ╔═══════════════════════════════════════════════════════════════════╗
    ║                    CYBERHEROES DELTA EXECUTOR v3.8               ║
    ║           Auto Win + Auto Task + ESP + Speed Boost +             ║
    ║            Stealth Invisibility + GOD MODE + Infinite Ammo +     ║
    ║            Auto Shield + Tpwalk + No Collision (NEW!)            ║
    ║              Survivor vs Killer - Generator Fixer                ║
    ║                   Developed by Deepseek-CH                       ║
    ║                     For Delta Executor                           ║
    ║   NEW: Tpwalk (slow movement when killer nearby)                 ║
    ║   NEW: No Collision (phase through killer when distance ≤30)     ║
    ╚═══════════════════════════════════════════════════════════════════╝
    ...
    ✅ TPWALK - Walkspeed reduced to 0.5 for 2 seconds when killer within 30 studs (NEW!)
    ✅ NO COLLISION - Phase through killer when within 30 studs (NEW!)
...
local PhysicsService = game:GetService("PhysicsService")          -- For collision groups
...
local config = {
    ...
    shieldEnabled = true,          -- Auto Shield toggle
    shieldRadius = 30,             -- Distance to activate shield
    tpwalkEnabled = true,          -- NEW: Tpwalk toggle
    tpwalkDuration = 2,            -- Duration for slow movement (seconds)
    tpwalkSlowSpeed = 0.5,         -- Speed when slowed
    noCollideEnabled = true,       -- NEW: No Collision toggle
    noCollideRadius = 30,          -- Distance to activate no collision
    guiVisible = true,
...
local shieldConnection = nil          -- Connection for shield monitoring
local currentForceField = nil         -- Reference to active ForceField
local isShieldActive = false          -- Shield state
local tpwalkConnection = nil          -- NEW: Connection for tpwalk monitoring
local isTpwalkActive = false          -- NEW: Tpwalk state
local noCollideConnection = nil       -- NEW: Connection for no collision monitoring
local isNoCollideActive = false       -- NEW: No collision state
local originalWalkSpeed = 16          -- Store original walkspeed for tpwalk
...
-- ============================================================================
-- FEATURE 1: AUTO WIN (EVENT-DRIVEN WITH ENHANCED DETECTION)
-- ============================================================================
...
-- ============================================================================
-- FEATURE 12: MODERN GUI WITH MINI LOGO (ENHANCED)
-- ============================================================================
...
local function createGUI()
    ...
    mainFrame.Size = UDim2.new(0, 260, 0, 460)  -- Increased height for new features
    mainFrame.Position = UDim2.new(0.85, -270, 0.5, -230)
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║                    CYBERHEROES DELTA EXECUTOR v3.8               ║")
