--[[
    ...
    ║           AUTO WIN (FINISH LINE) + ANTI HOOK + ESP +             ║
    ║            SPEED BOOST (TPWALK) + GOD MODE + INFINITE AMMO +     ║
    ║            AUTO SHIELD + NO COLLISION + MASS KILL +              ║
    ║            AUTO GENERATOR + SKILL CHECK + AUTO AIM               ║
    ║              Survivor vs Killer - Generator Fixer                ║
    ║                   Developed by Deepseek-CH                       ║
    ║                     For Delta Executor                           ║
    ║   NEW: Auto Win teleport ke finish line (escape)                 ║
    ║   NEW: Anti Hook - knockback saat digantung                     ║
    ║   NEW: ESP Hook, ESP Generator, ESP Player                      ║
    ║   NEW: TPWalk speed boost 2x (3 detik)                          ║
    ║   NEW: GUI ukuran 2x lebih kecil                                ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
if not _G.CyberHeroesState then
    _G.CyberHeroesState = {
        config = {
            ...
            highlightColorHook = Color3.fromRGB(255, 0, 0),
...
local isAntiHookActive = false
local hookKnockbackConnection = nil
local TouchID = 8822
...
-- ============================================================================
-- FEATURE 1: AUTO WIN (TELEPORT TO FINISH LINE / ESCAPE)
-- ============================================================================
local function findFinishLine()
    -- Cari finish line / gate / escape di workspace
    local finishLine = nil
    local containers = {Workspace, ReplicatedStorage, Lighting}
    local keywords = {"gate", "exit", "escape", "finish", "door", "portal", "end"}
        ...
        for _, obj in ipairs(container:GetDescendants()) do
            ...
            for _, kw in ipairs(keywords) do
                if name:find(kw) then
                    finishLine = obj
                    print("[AutoWin] Found finish line:", obj.Name)
                    return finishLine
                end
            end
        end
...
local function teleportToFinishLine()
    ...
    local finishLine = findFinishLine()
    if finishLine then
        local targetPos = finishLine:GetPivot().Position
        teleportTo(targetPos)
        print("[AutoWin] Teleported to finish line")
    else
        print("[AutoWin] Finish line not found, scanning...")
    end
end

local autoWinTimerConnection = nil
local function startAutoWin()
    if autoWinTimerConnection then return end
    autoWinTimerConnection = RunService.Heartbeat:Connect(function()
        teleportToFinishLine()
    end)
    print("[AutoWin] Auto win started (teleport to finish line)")
end
local function stopAutoWin()
    if autoWinTimerConnection then
        autoWinTimerConnection:Disconnect()
        autoWinTimerConnection = nil
    end
    print("[AutoWin] Auto win stopped")
end

-- ============================================================================
-- FEATURE 2: ANTI HOOK (Knockback saat digantung)
-- ============================================================================
local function applyKnockbackToKiller(killerCharacter)
    if not killerCharacter then return end
    local killerRoot = killerCharacter:FindFirstChild("HumanoidRootPart") or killerCharacter:FindFirstChild("Torso")
    if not killerRoot then return end

    -- Get direction away from survivor
    local direction = (killerRoot.Position - localRootPart.Position).Unit
    local knockbackForce = direction * 80 + Vector3.new(0, 20, 0)

    -- Apply knockback using BodyVelocity
    local bodyVel = Instance.new("BodyVelocity")
    bodyVel.MaxForce = Vector3.new(1e6, 1e6, 1e6)
    bodyVel.Velocity = knockbackForce
    bodyVel.Parent = killerRoot
    Debris:AddItem(bodyVel, 0.3)

    print("[AntiHook] Knockback applied to killer")
end

local function detectHookAndKnockback()
    if not config.antiHookEnabled then return end
    if not getLocalCharacter() then return end

    -- Detect jika player sedang di hook (tergantung)
    -- Check for being carried or hooked by killer
    local playerGui = localPlayer:FindFirstChild("PlayerGui")
    if playerGui then
        for _, gui in ipairs(playerGui:GetDescendants()) do
            if gui:IsA("TextLabel") or gui:IsA("Frame") then
                local name = gui.Name:lower()
                if name:find("hook") or name:find("carry") or name:find("struggle") then
                    if gui.Visible then
                        -- Find nearest killer
                        ...
                        if nearestKiller then
                            applyKnockbackToKiller(nearestKiller)
                            print("[AntiHook] Hook detected, knockback applied!")
                        end
                    end
                end
            end
...
local function startAntiHook()
    ...
    antiHookConnection = RunService.Heartbeat:Connect(function()
        detectHookAndKnockback()
    end)
    print("[AntiHook] Anti hook started")
end
local function stopAntiHook()
    ...
    print("[AntiHook] Anti hook stopped")
end

-- ============================================================================
-- FEATURE 3: ESP SYSTEM (Player, Generator, Hook)
-- ============================================================================
local function createPlayerHighlight(player)
    if playerEspHighlights[player.UserId] then
        if playerEspHighlights[player.UserId].Highlight then
            playerEspHighlights[player.UserId].Highlight:Destroy()
        end
        if playerEspHighlights[player.UserId].Billboard then
            playerEspHighlights[player.UserId].Billboard:Destroy()
        end
    ...
    highlight.Name = "CyberHeroes_PlayerESP"
    ...
    playerEspHighlights[player.UserId] = { Highlight = highlight, Billboard = billboard, NameLabel = nameLabel }
end

local function createGeneratorHighlight(generator)
...
local function createHookHighlight(hook)
    if hookEspHighlights[hook] then return end
    local highlight = Instance.new("Highlight")
    highlight.Name = "CyberHeroes_HookESP"
    highlight.FillColor = config.highlightColorHook
    highlight.FillTransparency = config.highlightTransparency
    highlight.OutlineColor = config.highlightColorHook
    highlight.OutlineTransparency = 0.2
    highlight.Adornee = hook
    highlight.Parent = hook
    hookEspHighlights[hook] = highlight
end
...
local function isHook(obj)
    if not obj then return false end
    local name = obj.Name:lower()
    return name:find("hook") or name:find("meat") or name:find("hang") or
           (obj:IsA("BasePart") and (name:find("hook") or name:find("meat")))
end

local function updateAllESP()
    if not config.espEnabled then
        -- Clear all ESPs
        for _, data in pairs(playerEspHighlights) do
            if data.Highlight then data.Highlight:Destroy() end
            if data.Billboard then data.Billboard:Destroy() end
        end
        playerEspHighlights = {}
        for _, highlight in pairs(generatorEspHighlights) do
            pcall(function() highlight:Destroy() end)
        end
        generatorEspHighlights = {}
        for _, highlight in pairs(hookEspHighlights) do
            pcall(function() highlight:Destroy() end)
        end
        hookEspHighlights = {}
        return
...
local function startESP()
    Players.PlayerAdded:Connect(function(player)
        if config.espEnabled then task.wait(0.5); createPlayerHighlight(player) end
    end)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer then
            player.CharacterAdded:Connect(function()
                if config.espEnabled then createPlayerHighlight(player) end
            end)
        end
    end
    updateAllESP()
    RunService.Heartbeat:Connect(function()
        if config.espEnabled then
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= localPlayer then
                    if not playerEspHighlights[player.UserId] or
                       (playerEspHighlights[player.UserId].Highlight and
                        playerEspHighlights[player.UserId].Highlight.Adornee ~= player.Character) then
                        createPlayerHighlight(player)
                    end
                end
            end
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if isGenerator(obj) and not generatorEspHighlights[obj] then
                    createGeneratorHighlight(obj)
                end
                if isHook(obj) and not hookEspHighlights[obj] then
                    createHookHighlight(obj)
                end
            end
        end
...
local function restartScript()
    ...
    if autoWinTimerConnection then autoWinTimerConnection:Disconnect(); autoWinTimerConnection = nil end
    ...
    processedGenerators = {}; playerEspHighlights = {}; generatorEspHighlights = {}; hookEspHighlights = {}
...
-- ============================================================================
-- FEATURE 10: TPWALK (Speed Boost 2x ketika killer berjarak ≤30 studs)
-- ============================================================================
local function applyTpwalkSpeedBoost()
    ...
    local boostedSpeed = originalWalkSpeed * config.tpwalkMultiplier
    localHumanoid.WalkSpeed = boostedSpeed
    isTpwalkActive = true
    print("[TPWalk] Speed boosted to " .. boostedSpeed .. " for " .. config.tpwalkDuration .. " seconds")
    ...
    print("[TPWalk] Speed restored to " .. originalWalkSpeed)
end
...
-- ============================================================================
-- FEATURE 12: MASS KILL LOOP (RANDOM SURVIVOR, TELEPORT BEHIND, PRESS E)
-- ============================================================================
...
-- ============================================================================
-- FEATURE 17: MODERN GUI (UKURAN 2x LEBIH KECIL, TAHAN CUTSCENE, MOBILE DRAG)
-- ============================================================================
...
local function createSettingsContent()
    ...
    colorRed.Size = UDim2.new(0, 50, 0, 20)
    ...
    colorCyan.Size = UDim2.new(0, 50, 0, 20)
    ...
    colorYellow.Size = UDim2.new(0, 50, 0, 20)
    ...
    chatLog.Size = UDim2.new(0.9, 0, 0, 80)
...
local function createPermanentTeleportButton()
    ...
    teleportButton.Size = UDim2.new(0, 40, 0, 40)
...
-- Main GUI (ukuran 2x lebih kecil)
local function createGUI()
    ...
    minimizeBtn.Position = UDim2.new(1, -40, 0, 1)
    minimizeBtn.Text = "−"
    ...
    contentPanel.Size = UDim2.new(1, -70, 1, -24)
    contentPanel.Position = UDim2.new(0, 65, 0, 24)
    contentPanel.BackgroundTransparency = 1
    contentPanel.Parent = mainFrame
    local gridLayout = Instance.new("UIGridLayout")
    gridLayout.CellSize = UDim2.new(0, 65, 0, 26)
    ...
    local features = {
        ...
        {name="antiHookEnabled", text="ANTI HOOK"},
    ...
    task.spawn(function()
        while screenGui and screenGui.Parent do
            local activeCount = (config.autoWinEnabled and 1 or 0) + (config.antiHookEnabled and 1 or 0) + (config.espEnabled and 1 or 0) +
...
local function restoreFeatureStates()
    ...
    if config.autoWinEnabled and not autoWinTimerConnection then
        startAutoWin()
    elseif not config.autoWinEnabled and autoWinTimerConnection then
        stopAutoWin()
    end
...
local function init()
    ...
    print("║        AUTO WIN (FINISH LINE) + ANTI HOOK + ESP HOOK             ║")
    print("║        SPEED BOOST TPWALK + GUI 2x LEBIH KECIL                   ║")
