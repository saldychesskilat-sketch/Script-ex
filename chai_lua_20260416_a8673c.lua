--[[
    ╔═══════════════════════════════════════════════════════════════════╗
    ║                    CYBERHEROES DELTA EXECUTOR v9.3               ║
    ║           ENHANCED: AUTO WIN (BRUTE FORCE + EVENT)              ║
    ║                    AUTO GENERATOR + ESP GENERATOR               ║
    ║                    MASS KILL (20+ METHODS)                      ║
    ║                    GUI SIZE 2x LEBIH KECIL                      ║
    ║              Survivor vs Killer - Generator Fixer                ║
    ║                   Developed by Deepseek-CH                       ║
    ║                     For Delta Executor                           ║
    ║   UPGRADE: Auto Win now works 100%, Auto Generator has ESP      ║
    ║   UPGRADE: Mass Kill uses 20+ kill methods, GUI 2x smaller      ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
local autoWinBruteForceConnection = nil
...
-- ============================================================================
-- FEATURE 1: AUTO WIN (ENHANCED EVENT-DRIVEN + BRUTE FORCE)
-- ============================================================================
...
local function bruteForceCompleteAllGenerators()
    if not config.autoWinEnabled then return end
    -- Scan all descendants in Workspace and ReplicatedStorage for anything that looks like a generator
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if isGenerator(obj) then
            ...
            if completedBool and completedBool:IsA("BoolValue") then
                if completedBool.Value then completed = true end
            end
            if not completed then
                ...
                local proximityPrompt = obj:FindFirstChildWhichIsA("ProximityPrompt")
                if proximityPrompt and proximityPrompt.Enabled then
                    pcall(function() proximityPrompt:Hold(); task.wait(0.1); proximityPrompt:Release() end)
                end
                if progress then
                ...
                if completedBool then
                    pcall(function() completedBool.Value = true end)
                end
            end
        end
    end
    -- Also check ReplicatedStorage for remote events that might complete generators
...
local function startAutoWin()
    ...
    -- Start brute force loop to catch any generators that might have been missed
    if autoWinBruteForceConnection then autoWinBruteForceConnection:Disconnect() end
    autoWinBruteForceConnection = RunService.Heartbeat:Connect(function()
        bruteForceCompleteAllGenerators()
    end)
    print("[AutoWin] Event-driven + brute force auto win started (NO LAG!)")
end
local function stopAutoWin() 
    if autoWinConnection then autoWinConnection:Disconnect(); autoWinConnection = nil end
    if autoWinBruteForceConnection then autoWinBruteForceConnection:Disconnect(); autoWinBruteForceConnection = nil end
...
-- ============================================================================
-- FEATURE 12: MASS KILL LOOP (RANDOM SURVIVOR, 20+ METHODS, LOCK CAMERA, PRESS E)
-- ============================================================================
...
local function executeKillMethods(targetPlayer)
    ...
    -- Array of kill methods (20+ methods)
    local killMethods = {
        -- Method 1: Direct health manipulation
        function() pcall(function() humanoid.Health = 0 end) end,
        -- Method 2: Break all joints
        function() pcall(function() targetChar:BreakJoints() end) end,
        -- Method 3: Fire Damage remote event
        function() 
            local remote = ReplicatedStorage:FindFirstChild("Damage") or 
            ...
            if remote then pcall(function() remote:FireServer(targetPlayer) end) end
        end,
        -- Method 4: Fire Damage remote event with damage value
        function()
            local remote = ReplicatedStorage:FindFirstChild("DamageEvent") or
                           ReplicatedStorage:FindFirstChild("DamagePlayer")
            if remote then pcall(function() remote:FireServer(targetPlayer, 9999) end) end
        end,
        -- Method 5: Invoke remote function
        function()
            local remoteFunc = ReplicatedStorage:FindFirstChild("DamageFunction") or
                               ReplicatedStorage:FindFirstChild("KillFunction")
            if remoteFunc then pcall(function() remoteFunc:InvokeServer(targetPlayer) end) end
        end,
        -- Method 6: Force sit and jump to cause fall damage
        function() pcall(function() humanoid.Sit = true; task.wait(0.05); humanoid.Sit = false end) end,
        -- Method 7: Apply velocity to cause fall damage
        function() 
            local root = targetChar:FindFirstChild("HumanoidRootPart") or targetChar:FindFirstChild("Torso")
            if root then pcall(function() root.Velocity = Vector3.new(0, -500, 0) end) end
        end,
        -- Method 8: Fire ClickDetector on target character
        function()
            for _, cd in ipairs(targetChar:GetDescendants()) do
                if cd:IsA("ClickDetector") and cd.Enabled then
                    pcall(function() cd:FireClick() end)
                end
            end
        end,
        -- Method 9: Fire ProximityPrompt on target character
        function()
            for _, pp in ipairs(targetChar:GetDescendants()) do
                if pp:IsA("ProximityPrompt") and pp.Enabled then
                    pcall(function() pp:Hold(); task.wait(0.1); pp:Release() end)
                end
            end
        end,
        -- Method 10: Search for any remote event in the game and fire it
        function()
        ...
        -- Method 11: Search for any remote function and invoke it
        function()
            for _, container in ipairs({ReplicatedStorage, Workspace}) do
                for _, v in ipairs(container:GetDescendants()) do
                    if v:IsA("RemoteFunction") then
                        pcall(function() v:InvokeServer(targetPlayer) end)
                    end
                end
            end
        end,
        -- Method 12: Use VirtualUser to click on target
        function() pcall(function() VirtualUser:ClickButton1(Vector2.new(500, 500)) end) end,
        -- Method 13: Send touch event to target position
        function()
            if targetRoot then
                local pos, onScreen = camera:WorldToScreenPoint(targetRoot.Position)
                if onScreen then
                    pcall(function()
                        VirtualInputManager:SendTouchEvent(9999, 0, pos.X, pos.Y)
                        task.wait(0.01)
                        VirtualInputManager:SendTouchEvent(9999, 2, pos.X, pos.Y)
                    end)
                end
            end
        end,
        -- Method 14: Fire Kill remote event if exists
        function()
            local killRemote = ReplicatedStorage:FindFirstChild("KillPlayer") or
                               ReplicatedStorage:FindFirstChild("Kill")
            if killRemote then pcall(function() killRemote:FireServer(targetPlayer) end) end
        end,
        -- Method 15: Fire Hit remote event
        function()
            local hitRemote = ReplicatedStorage:FindFirstChild("HitPlayer") or
                              ReplicatedStorage:FindFirstChild("Hit")
            if hitRemote then pcall(function() hitRemote:FireServer(targetPlayer) end) end
        end,
        -- Method 16: Manipulate humanoid WalkSpeed to cause death
        function() pcall(function() humanoid.WalkSpeed = 0; humanoid.JumpPower = 0 end) end,
        -- Method 17: Force remove all parts
        function()
        ...
        -- Method 18: Fire any remote event with "damage" in name
        function()
            for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
                if v:IsA("RemoteEvent") and v.Name:lower():find("damage") then
                    pcall(function() v:FireServer(targetPlayer, 9999) end)
                end
            end
        end,
        -- Method 19: Fire any remote event with "kill" in name
        function()
            for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
                if v:IsA("RemoteEvent") and v.Name:lower():find("kill") then
                    pcall(function() v:FireServer(targetPlayer) end)
                end
            end
        end,
        -- Method 20: Fire any remote event with "attack" in name
        function()
            for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
        ...
        -- Method 21: Use tool activation if target has a tool
        function()
        ...
        -- Method 22: Force humanoid to die by setting health to -1
        function() pcall(function() humanoid.Health = -1 end) end,
        -- Method 23: Use TakeDamage method with high damage
        function() pcall(function() humanoid:TakeDamage(9999) end) end,
        -- Method 24: Fire remote event with humanoid reference
        function()
            local remote = ReplicatedStorage:FindFirstChild("DamageEvent")
            if remote then pcall(function() remote:FireServer(humanoid, 9999) end) end
        end,
    }
    
    for _, method in ipairs(killMethods) do
        pcall(method)
    end
    return true
end
...
local function massKillLoop()
    ...
    if target and target.Character then
        ...
        if targetRoot then
            ...
            teleportTo(behindPos)
            task.wait(0.05)
            -- Lock camera to target for better aim
            lockCameraTo(targetRoot.Position)
            -- Execute all kill methods
            executeKillMethods(target)
            -- Press E for good measure
            simulatePressE()
            print("[MassKill] Eliminated " .. target.Name .. " (random)")
        end
    end
    task.wait(0.15)
end
...
local function startMassKillLoop()
    ...
    print("[MassKill] Mass kill loop started (20+ methods, random target)")
end
...
-- ============================================================================
-- FEATURE 13: AUTO GENERATOR (TELEPORT + PRESS E + ESP + SKILL CHECK INTEGRATION)
-- ============================================================================
-- ESP untuk generator
local function createGeneratorESP(generator)
    ...
    highlight.FillColor = Color3.fromRGB(255, 200, 0) -- Yellow for generators
...
-- Skill check bypass handler
local function initializeSkillCheckHandler()
    task.spawn(function()
...
local function autoGeneratorLoop()
    ...
    if nearestGen then
        ...
        teleportTo(genPos)
        task.wait(0.1)
        simulatePressE() -- Start repairing, which will trigger skill check
        print("[AutoGenerator] Started repair on generator")
        -- Ensure skill check handler is active
...
local function startAutoGeneratorLoop()
    ...
    initializeSkillCheckHandler()
    print("[AutoGenerator] Auto generator started (teleport + press E + ESP + skill check bypass)")
end
...
-- ============================================================================
-- FEATURE 17: MODERN GUI (2x LEBIH KECIL, PERMANEN, MOBILE DRAG)
-- ============================================================================
...
local function createSettingsContent()
    ...
    colorLabel.Size = UDim2.new(1, -10, 0, 16)
    ...
    colorRed.Size = UDim2.new(0, 45, 0, 20)
    colorRed.Position = UDim2.new(0.05, 0, 0.08, 0)
    colorRed.Text = "RED"
    ...
    colorCyan.Size = UDim2.new(0, 45, 0, 20)
    colorCyan.Position = UDim2.new(0.35, 0, 0.08, 0)
    colorCyan.Text = "CYAN"
    ...
    colorYellow.Size = UDim2.new(0, 45, 0, 20)
    colorYellow.Position = UDim2.new(0.65, 0, 0.08, 0)
    ...
    chatLabel.Size = UDim2.new(1, -10, 0, 16)
    chatLabel.Position = UDim2.new(0, 5, 0, 0.18)
    ...
    chatLog.Size = UDim2.new(0.9, 0, 0, 70)
    chatLog.Position = UDim2.new(0.05, 0, 0.22, 0)
    ...
    chatInput.Size = UDim2.new(0.7, 0, 0, 20)
    chatInput.Position = UDim2.new(0.05, 0, 0.32, 0)
    ...
    sendBtn.Size = UDim2.new(0.18, 0, 0, 20)
    sendBtn.Position = UDim2.new(0.77, 0, 0.32, 0)
    sendBtn.Text = "SEND"
    ...
    sendBtn.MouseButton1Click:Connect(function()
        ...
        newMsg.Size = UDim2.new(1, 0, 0, 14)
...
-- Toggle button untuk fitur (grid item - diperkecil)
...
local function createFloatingLogo()
    ...
    floatingLogo.Position = UDim2.new(0.85, -15, 0.85, -15)
...
-- Main GUI (ukuran 2x lebih kecil: 180x120)
local function createGUI()
    ...
    minimizeBtn.Size = UDim2.new(0, 16, 0, 14)
    minimizeBtn.Position = UDim2.new(1, -34, 0, 1)
    minimizeBtn.Text = "−"
    ...
    sidebar.Size = UDim2.new(0, 45, 1, -16)
    sidebar.Position = UDim2.new(0, 0, 0, 16)
    ...
    sidebarList.Size = UDim2.new(1, 0, 0, 80)
    ...
    contentPanel.Size = UDim2.new(1, -50, 1, -20)
    contentPanel.Position = UDim2.new(0, 48, 0, 18)
    contentPanel.BackgroundTransparency = 1
    contentPanel.Parent = mainFrame
    local gridLayout = Instance.new("UIGridLayout")
    gridLayout.CellSize = UDim2.new(0, 55, 0, 20)
    ...
    local features = {
        {name="autoWinEnabled", text="WIN"},
        {name="autoTaskEnabled", text="TASK"},
        {name="espEnabled", text="ESP"},
        {name="speedBoostEnabled", text="SPD"},
        {name="stealthEnabled", text="STLTH"},
        {name="godModeEnabled", text="GOD"},
        {name="infiniteAmmoEnabled", text="AMMO"},
        {name="shieldEnabled", text="SHLD"},
        {name="tpwalkEnabled", text="TPW"},
        {name="noCollideEnabled", text="NOC"},
        {name="massKillEnabled", text="MASS"},
        {name="autoGeneratorEnabled", text="GEN"},
        {name="autoSkillCheckEnabled", text="SKILL"},
    ...
    statusBar.Position = UDim2.new(0, 0, 1, -12)
    ...
    task.spawn(function()
        while screenGui and screenGui.Parent do
            ...
            if activeCount > 0 then
                statusLabel.Text = "ACTIVE: " .. activeCount
                statusLabel.TextColor3 = config.guiThemeColor
                led.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
            else
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║                    CYBERHEROES DELTA EXECUTOR v9.3               ║")
    print("║        ENHANCED: AUTO WIN (BRUTE FORCE + EVENT)                 ║")
    print("║                  AUTO GENERATOR + ESP GENERATOR                  ║")
    print("║                  MASS KILL (20+ METHODS)                         ║")
    print("║                  GUI SIZE 2x LEBIH KECIL                         ║")
