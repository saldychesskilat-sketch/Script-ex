--[[
    ╔═══════════════════════════════════════════════════════════════════╗
    ║                    CYBERHEROES DELTA EXECUTOR v10.0              ║
    ...
    ║            AUTO AIM (KILLER LOCK) + TELEPORT TO NEAREST SURVIVOR ║
    ║              Survivor vs Killer - Generator Fixer                ║
    ║                   Developed by Deepseek-CH                       ║
    ║                     For Delta Executor                           ║
    ║   FIXED: Auto Win (finishline teleport)                         ║
    ║   FIXED: Auto Task (anti-hook + auto 1x mode)                   ║
    ║   FIXED: Auto Task (lever goal gate system)                     ║
    ║   FIXED: Auto Generator (ESP to survivor, killer, hook)         ║
    ║   FIXED: Tpwalk (2x speed boost when killer within 30 studs)    ║
    ║   FIXED: Auto Kill (hilt hitbox detection)                      ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
-- ============================================================================
-- FEATURE 1: AUTO WIN (TELEPORT TO FINISHLINE)
-- ============================================================================
local function getFinishlinePosition()
    local finishline = Workspace:FindFirstChild("fininshline")
    if finishline then
    ...
    local finishlineAlt = Workspace:FindFirstChild("FinishLine") or Workspace:FindFirstChild("EscapeZone") or Workspace:FindFirstChild("ExitGate")
    if finishlineAlt then
        if finishlineAlt:IsA("BasePart") then
            return finishlineAlt.Position
        elseif finishlineAlt:FindFirstChildWhichIsA("BasePart") then
            return finishlineAlt:FindFirstChildWhichIsA("BasePart").Position
        end
    end
    return nil
end
...
local function startAutoWin()
    ...
    print("[AutoWin] Auto win started (teleport to finishline)")
end
...
-- ============================================================================
-- FEATURE 2: AUTO TASK (LEVER GOAL GATE SYSTEM + ANTI-HOOK)
-- ============================================================================
local function isPlayerHooked()
    if not localCharacter then return false end
    local hook = localCharacter:FindFirstChild("Hook") or localCharacter:FindFirstChild("Hooked")
...
local function rescueFromHook()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer then
            ...
            if char then
                ...
                if isKiller and (char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Torso")) then
                    killer = char
                    break
                end
            end
    ...
    if killer then
        local killerRoot = killer:FindFirstChild("HumanoidRootPart") or killer:FindFirstChild("Torso")
        if killerRoot then
            local killerPos = killerRoot.Position
            local direction = (localRootPart.Position - killerPos).Unit
            local knockbackForce = direction * 50 + Vector3.new(0, 20, 0)
            if localRootPart then
                localRootPart.Velocity = knockbackForce
            end
...
local function findLeverGoal()
    local leverGoal = Workspace:FindFirstChild("LeverGoal")
    if not leverGoal then
...
local function findGateF_O()
    local gateFO = Workspace:FindFirstChild("F_O")
    if not gateFO then
        for _, obj in ipairs(Workspace:GetDescendants()) do
...
local function findRightGate()
    local rightGate = Workspace:FindFirstChild("RightGate")
    if not rightGate then
        for _, obj in ipairs(Workspace:GetDescendants()) do
...
local function findLiftGate()
    local liftGate = Workspace:FindFirstChild("LiftGate")
    if not liftGate then
        for _, obj in ipairs(Workspace:GetDescendants()) do
...
local function interactWithGate(gate)
    if not gate or not gate:IsA("BasePart") then return false end
    local clickDetector = gate:FindFirstChildWhichIsA("ClickDetector")
    if clickDetector and clickDetector.Enabled then
        pcall(function() clickDetector:FireClick() end)
        return true
    end
    local proximityPrompt = gate:FindFirstChildWhichIsA("ProximityPrompt")
...
local function startAutoTask()
    ...
    print("[AutoTask] Auto task started (lever goal + anti-hook + auto 1x mode)")
end
...
-- ============================================================================
-- FEATURE 10: TPWALK (2x speed boost when killer within 30 studs)
-- ============================================================================
local function applyTpwalkBoost()
    ...
    isTpwalkActive = true
    print("[Tpwalk] Speed boosted to " .. boostSpeed .. " for " .. config.tpwalkDuration .. " seconds")
    task.wait(config.tpwalkDuration)
    if localHumanoid then localHumanoid.WalkSpeed = originalTpwalkSpeed end
    isTpwalkActive = false
    print("[Tpwalk] Speed restored to " .. originalTpwalkSpeed)
end
...
local function startTpwalkMonitor()
    ...
    print("[Tpwalk] Monitor started (2x speed when killer ≤ 30 studs)")
end
...
-- ============================================================================
-- FEATURE 12: MASS KILL LOOP (USING HILT HITBOX)
-- ============================================================================
        ...
        if targetRoot then
            ...
            if hilt and hilt:IsA("BasePart") then
                ...
                if clickDetector and clickDetector.Enabled then
                    ...
                    if proximityPrompt and proximityPrompt.Enabled then
                        pcall(function() proximityPrompt:Hold(); task.wait(0.1); proximityPrompt:Release() end)
                        print("[MassKill] Hit " .. target.Name .. " using hilt proximity prompt")
                    else
                        local touchInterest = Instance.new("TouchInterest")
                        touchInterest.Parent = hilt
                        task.wait(0.1)
                        touchInterest:Destroy()
                        print("[MassKill] Hit " .. target.Name .. " using hilt touch interest")
                    end
                end
            else
                simulatePressE()
...
local function startMassKillLoop()
    ...
    print("[MassKill] Mass kill loop started (using hilt hitbox)")
end
...
local function createHookESP(hook)
    if generatorEspHighlights[hook] then return end
    local highlight = Instance.new("Highlight")
    ...
    generatorEspHighlights[hook] = highlight
end
...
local function updateAutoGeneratorESP()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer and player.Character then
            ...
            if not generatorEspHighlights[player.Character] then
                local highlight = Instance.new("Highlight")
                ...
                generatorEspHighlights[player.Character] = highlight
            end
        end
    end
end
...
local function startAutoGeneratorLoop()
    ...
    print("[AutoGenerator] Auto generator started (with ESP for generators, survivors, killers, hooks)")
end
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║                    CYBERHEROES DELTA EXECUTOR v10.0              ║")
    print("║        Auto Win (finishline teleport)                            ║")
    print("║        Auto Task (lever goal + anti-hook + auto 1x mode)         ║")
    print("║        Auto Generator (ESP to survivor, killer, hook)            ║")
    print("║        Tpwalk (2x speed when killer within 30 studs)             ║")
    print("║        Auto Kill (hilt hitbox detection)                         ║")
