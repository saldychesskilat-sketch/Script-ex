--[[
    CYBERHEROES AGGRESSIVE AIM v5.0 - PREMIUM EDITION
    - Fixed ALL player detection
    - Auto relock on death / out of range
    - X-axis correction system
    - Smart prediction (idle/jump/sprint)
    - Multi-target tracking + lock
    - Team system (friend/foe)
    - Visual hit indicator, FOV animation, lock-on effect
    - Optimized performance (cached raycast, no FPS drop)
    - Legit / Rage mode
...
local Config = {
    -- Main
    enabled = true,
    aimMode = "rage",              -- "legit" or "rage"
    teamCheck = false,             -- false = semua, true = bedakan tim
    myTeamColor = nil,             -- auto detect
    ...
    showDistance = true,
    showName = true,
    espBoxStyle = "square",        -- "square" or "rounded"
    espColors = {
    ...
    -- Aiming
    aimFOV = 200,
    smoothVisible = 0.03,          -- rage mode uses lower
    smoothOccluded = 0.12,
    snapDistanceThreshold = 30,
    predictionMultiplier = 0.25,
    aimPartPriority = {"Head", "UpperTorso", "HumanoidRootPart"},
    randomOffset = 0.5,            -- human-like
    xAxisCorrection = 0.6,         -- 0-1, semakin tinggi semakin cepat center
...
local myTeam = nil
local behindEffectFrames = {}  -- for gradient
local enemyList = {}            -- cache all alive players

-- GUI Elements
local guiMainFrame = nil
local guiVisible = true
local espToggle, aimToggle, legitBtn, rageBtn, fovSlider, smoothSlider, fovFill, smoothFill
local fovLabel, smoothLabel, lockStatusLabel, behindLabel
local teamCheckToggle, espColorBtn

-- Raycast cache
local raycastParams = RaycastParams.new()
local lastRaycastResults = {}
...
local function isAlive(player)
    ...
    return hum and hum.Health > 0 and char.Parent == workspace
end
...
local function getTeam(player)
    ...
    -- fallback: cari nilai team dalam leaderstats atau atribut lain
...
local function isVisible(part, character)
    ...
    local cacheKey = tostring(part) .. tostring(tick() // 0.1)
    if Config.cacheRaycast and lastRaycastResults[cacheKey] ~= nil then
        return lastRaycastResults[cacheKey]
    end
    ...
    if Config.cacheRaycast then
        lastRaycastResults[cacheKey] = visible
        task.delay(0.1, function() lastRaycastResults[cacheKey] = nil end)
    end
    return visible
end

-- ============================================================================
-- GET BEST TARGET PART (with fallback)
-- ============================================================================
...
-- ============================================================================
-- SMART TARGET SELECTION (Priority: visible > distance > screen center)
-- ============================================================================
            ...
            if char then
                ...
                if targetPart then
                    ...
                    if screenPos then
                        ...
                        -- Scoring: visible players get huge bonus
                        local score = distFromCenter
                        if not visible then
                            score = score + 10000
                        end
                        -- distance bonus (closer = lower score)
                        local worldDist = (camera.CFrame.Position - targetPart.Position).Magnitude
                        score = score + worldDist * 0.5
...
local function shouldKeepTarget(targetPlayer)
    ...
    local center = Vector2.new(camera.ViewportSize.X / 2, camera.ViewportSize.Y / 2)
    local distFromCenter = (screenPos - center).Magnitude
    if distFromCenter > Config.aimFOV * 1.5 then
        return false
    end
    return true
end
...
-- ============================================================================
-- SMART PREDICTION (velocity + movement type)
-- ============================================================================
...
local function predictPosition(part, player)
    ...
    -- Deteksi gerakan khusus
    local char = player.Character
    local humanoid = char and char:FindFirstChildOfClass("Humanoid")
    if humanoid then
        if humanoid.Jump then
            multiplier = multiplier * 1.5   -- lompat lebih jauh
        elseif speed > 30 then
            multiplier = multiplier * 1.2   -- sprint
        elseif speed < 5 then
            multiplier = multiplier * 0.5   -- diam/berjalan lambat
        end
    end
...
-- X-AXIS CORRECTION SYSTEM
-- ============================================================================
local function applyXAxisCorrection(targetScreenPos)
    local centerX = camera.ViewportSize.X / 2
    local deltaX = targetScreenPos.X - centerX
    if math.abs(deltaX) > 5 then
        local correction = deltaX * Config.xAxisCorrection * (Config.aimMode == "rage" and 1.5 or 1)
        mousemoverel(correction, 0)
    end
end

-- ============================================================================
-- AIM LOCK (with smoothing, prediction, x-axis correction)
-- ============================================================================
local function updateAim(deltaTime)
    ...
    -- Aim if target exists
    if currentTarget and currentTargetPart then
        local predictedPos = predictPosition(currentTargetPart, currentTarget)
        if predictedPos then
            ...
            if screenPos then
                ...
                -- Smoothness based on mode and visibility
                ...
                if Config.aimMode == "rage" then
                    ...
                    if currentTargetVisible then
                        ...
                        smooth = Config.smoothOccluded
                    end
                end
                
                -- Human-like offset (only in legit mode)
                local offsetX, offsetY = 0, 0
                if Config.aimMode == "legit" then
                    offsetX = (math.random() - 0.5) * Config.randomOffset
                    offsetY = (math.random() - 0.5) * Config.randomOffset
                end
                ...
                if finalDelta.Magnitude > 0.3 then
                    ...
                    -- X-axis correction
                    applyXAxisCorrection(screenPos)
                end
                
                -- Hit indicator jika aim tepat sasaran (distance small)
                if distancePixels < 20 and Config.hitIndicatorDuration > 0 then
                    showHitIndicator()
                end
            end
        end
...
local function createLockEffect(targetPart)
    ...
    weld.Parent = part
    lockEffectPart = part
    TweenService:Create(part, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = Vector3.new(2, 2, 2)}):Play()
...
-- ============================================================================
-- ENEMY BEHIND EFFECT (gradient merah di sisi layar)
-- ============================================================================
local behindGradient = nil
local function updateEnemyBehindEffect()
    if not Config.enemyBehindEffect then
        ...
        if not isTeammate(player) then
            ...
            if targetPart then
                ...
                if screenPos then
                    ...
                    local isTeammatePlayer = isTeammate(player)
                    
                    local boxColor
                    if isLocked then
                        boxColor = Config.espColors.locked
                    elseif isTeammatePlayer then
                        boxColor = Config.espColors.teammate
                    elseif visible then
                    ...
                    if Config.showName then
                        esp.name.Text = player.Name .. (isTeammatePlayer and " (TEAM)" or "")
...
local function createModernGUI()
    ...
    title.Text = "🔥 AGGRESSIVE AIM v5.0 🔥"
    ...
    smoothLabel.Text = "SMOOTH: " .. string.format("%.2f", Config.smoothVisible)
...
RunService.RenderStepped:Connect(function()
    ...
    updateFOVCircle()
    updateAim(deltaTime)
    updateEnemyBehindEffect()
    
    -- Lock effect visual (refresh setiap lock)
...
print("🔥 CYBERHEROES AGGRESSIVE AIM v5.0 PREMIUM loaded! 🔥")
print("F = ESP | V = Auto Aim | RightShift = Toggle GUI")
print("Legit/Rage mode available in GUI | Team check available")