...
local function flingPlayer(targetPlayer)
    ...
    if rootPart then
        -- Hitung vektor fling
        local flingVector = Vector3.new(0, 0, 0)
        
        if config.flingRandomDirection then
            -- Arah horizontal acak
            local angle = math.random() * 2 * math.pi
            local horizX = math.cos(angle)
            local horizZ = math.sin(angle)
            flingVector = Vector3.new(horizX, 0, horizZ) * config.flingForce
        else
            -- Arah depan (dari player) + ke atas
            local forward = localRootPart and localRootPart.CFrame.LookVector or Vector3.new(0,0,1)
            flingVector = forward * config.flingForce
        end
        
        if config.flingUpward then
            flingVector = flingVector + Vector3.new(0, config.flingForce * 0.5, 0)
        end
        
        -- Terapkan velocity
        pcall(function()
            rootPart.Velocity = flingVector
            debugPrint(string.format("Fling applied to %s: %s", targetPlayer.Name, tostring(flingVector)))
        end)
        success = true
    end
    
    -- Cadangan: kill humanoid jika fling gagal
    ...
    -- Hancurkan joints sebagai fallback tambahan
    pcall(function()
    ...
    pcall(function()
        localPlayer:Chat("⚡ " .. targetPlayer.Name .. " terkena CyberHeroes Fling!")
    end)
    
    debugPrint("Fling attempted on " .. targetPlayer.Name .. ", success: " .. tostring(success))
    return success
end
...
_G.CyberHeroes = {
    ...
    setFlingForce = function(force)
        config.flingForce = math.max(100, force)
        createNotification("💥 Fling Force", tostring(config.flingForce), 2)
    end,
    
    toggleUpward = function()
        config.flingUpward = not config.flingUpward
        createNotification("⬆️ Upward", config.flingUpward and "ON" or "OFF", 1)
    end,
    
    toggleRandomDirection = function()
        config.flingRandomDirection = not config.flingRandomDirection
        createNotification("🎲 Random Direction", config.flingRandomDirection and "ON" or "OFF", 1)
    end,
    ...
    getStatus = function()
        ...
        print("Upward: " .. tostring(config.flingUpward))
        print("Random Direction: " .. tostring(config.flingRandomDirection))
...
local function initialize()
    print("\n╔════════════════════════════════════════════════════════════╗")
    print("║   💀 CYBERHEROES DELTA EXECUTOR v9.0 - FLING FORCE         ║")
    print("║   🔥 REAL Forced Respawn: Massive Fling Velocity            ║")
    print("║   ⚡ Launch players out of map for guaranteed death        ║")
    ...
    print("[✓] Upward: " .. tostring(config.flingUpward))
    print("[✓] Random Direction: " .. tostring(config.flingRandomDirection))
    print("[✓] Humanoid Kill Backup: " .. tostring(config.enableHumanoidKill))
    print("[✓] Real server-side fling ensures death & respawn\n")
    
    if localPlayer.Character then
        localCharacter = localPlayer.Character
        createRangeIndicator()
        createNotification("💀 CYBERHEROES ACTIVE", "Approach other players to FLING them away!", 4)
        detectionConnection = RunService.RenderStepped:Connect(checkProximity)
    end
    ...
    localPlayer.CharacterAdded:Connect(function(char)
        localCharacter = char
        createRangeIndicator()
        createNotification("💀 CYBERHEROES ACTIVE", "Approach other players to FLING them away!", 3)
    end)
end
...
-- ============================================
-- END OF SCRIPT v9.0
-- Uses massive fling velocity to launch players out of map
-- Guarantees server-side death and respawn
-- ============================================        end
        local offset = Vector3.new(
            (math.random() - 0.5) * intensity,
            (math.random() - 0.5) * intensity,
