--[[
    ╔═══════════════════════════════════════════════════════════════════╗
    ║                    CYBERHEROES DELTA EXECUTOR v8.0               ║
    ...
    ║                     For Delta Executor                           ║
    ║   NEW: Auto Skill Check (VirtualInputManager method)            ║
    ║   NEW: Auto Aim (lock camera to nearest killer)                 ║
    ║   NEW: Teleport to Nearest Player (button)                      ║
    ║   FIXED: GUI layout 3 rows, smaller buttons                     ║
    ║   FIXED: Auto Win performance (no black screen)                 ║
    ╚═══════════════════════════════════════════════════════════════════╝
--]]
...
-- ============================================================================
-- FEATURE 13: AUTO GENERATOR + SKILL CHECK BYPASS (FROM VD SCRIPT)
-- ============================================================================
...
-- Skill Check Bypass using VirtualInputManager (from VD script)
...
local function startAutoSkillCheck()
    ...
    autoSkillCheckConnection = RunService.Heartbeat:Connect(function()
        ...
        if not getLocalCharacter() then return end
        -- Also try generic method: find skill check GUI
        local playerGui = localPlayer:FindFirstChild("PlayerGui")
        if playerGui then
...
-- ============================================================================
-- FEATURE 14: AUTO AIM (LOCK CAMERA TO NEAREST KILLER)
-- ============================================================================
...
local function startAutoAim()
    ...
    print("[AutoAim] Auto aim started (lock to nearest killer)")
end
...
-- ============================================================================
-- FEATURE 15: TELEPORT TO NEAREST PLAYER (BUTTON)
-- ============================================================================
...
-- ============================================================================
-- FEATURE 16: MODERN GUI WITH MINI LOGO (3 ROWS, SMALL BUTTONS)
-- ============================================================================
...
local function createSmallCircleButton(parent, name, text, callback)
    local button = Instance.new("TextButton")
    button.Name = name
...
local function createGUI()
    ...
    mainFrame.Position = UDim2.new(0.5, -250, 0.5, -80)
    ...
    innerGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(30,5,10)), ColorSequenceKeypoint.new(1, Color3.fromRGB(10,0,2))})
    ...
    row2Layout.Padding = UDim.new(0, 4)
    row2Layout.Parent = row2
    local row3 = Instance.new("Frame")
    row3.Size = UDim2.new(1, 0, 0, 32)
    row3.Position = UDim2.new(0, 0, 0, 72)
    ...
    -- Row 1: Auto Win, Auto Task, ESP, Speed, Stealth
    createToggleButton(row1, "autoWinEnabled", "AUTO WIN", config.autoWinEnabled)
    createToggleButton(row1, "autoTaskEnabled", "AUTO TASK", config.autoTaskEnabled)
    createToggleButton(row1, "espEnabled", "ESP", config.espEnabled)
    createToggleButton(row1, "speedBoostEnabled", "SPEED", config.speedBoostEnabled)
    createToggleButton(row1, "stealthEnabled", "STEALTH", config.stealthEnabled)
    -- Row 2: God Mode, AMMO, Shield, Tpwalk, No Collide, Mass Kill
    createToggleButton(row2, "godModeEnabled", "GOD MODE", config.godModeEnabled)
    createToggleButton(row2, "infiniteAmmoEnabled", "AMMO", config.infiniteAmmoEnabled)
    createToggleButton(row2, "shieldEnabled", "SHIELD", config.shieldEnabled)
    createToggleButton(row2, "tpwalkEnabled", "TPWALK", config.tpwalkEnabled)
    createToggleButton(row2, "noCollideEnabled", "NO COLLIDE", config.noCollideEnabled)
    createToggleButton(row2, "massKillEnabled", "MASS KILL", config.massKillEnabled)
    -- Row 3: Auto Gen, Skill Check, Auto Aim, Execute, Restart
    createToggleButton(row3, "autoGeneratorEnabled", "AUTO GEN", config.autoGeneratorEnabled)
    createToggleButton(row3, "autoSkillCheckEnabled", "SKILL CHECK", config.autoSkillCheckEnabled)
    createToggleButton(row3, "autoAimEnabled", "AUTO AIM", config.autoAimEnabled)
    createToggleButton(row3, "executeMassKill", "EXECUTE", false)
    createToggleButton(row3, "restartScript", "RESTART", false)
    -- Two small circle buttons for teleport and maybe something else
    local circlePanel = Instance.new("Frame")
    circlePanel.Size = UDim2.new(0, 120, 0, 60)
    circlePanel.Position = UDim2.new(1, -130, 0, 5)
    circlePanel.BackgroundTransparency = 1
    circlePanel.Parent = mainFrame
    createSmallCircleButton(circlePanel, "teleportNearest", "TP", teleportToNearestPlayer)
    ...
    if input.KeyCode == config.guiToggleKey then
        ...
        if mainFrame then
            ...
            if config.guiVisible then
                if floatingLogo then floatingLogo.Visible = false; isLogoVisible = false end
            else
                if not isLogoVisible then floatingLogo = createFloatingLogo(); floatingLogo.Visible = true; isLogoVisible = true end
            end
        end
    end
end)
...
local function init()
    print("╔═══════════════════════════════════════════════════════════════════╗")
    print("║                    CYBERHEROES DELTA EXECUTOR v8.0               ║")
