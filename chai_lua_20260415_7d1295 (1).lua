...
if not _G.CyberHeroesState then
    _G.CyberHeroesState = {
        ...
        featuresActive = {}  -- akan diisi saat start/stop
    }
end
...
local settingsContentCreated = false
...
local function createPermanentTeleportButton()
    ...
    teleportButton.InputBegan:Connect(onInputBegan)
    teleportButton.InputChanged:Connect(onInputChanged)
    teleportButton.InputEnded:Connect(onInputEnded)
end
...
local function createGUI()
    ...
    -- Draggable window (smooth untuk mobile & PC)
    local dragging = false
    local dragStart, startPos
    ...
    mainFrame.InputBegan:Connect(onMainInputBegan)
    mainFrame.InputChanged:Connect(onMainInputChanged)
    mainFrame.InputEnded:Connect(onMainInputEnded)
...
local function ensureGUIPersistent()
    task.spawn(function()
        while isScriptRunning do
            -- Cek dan recover screenGui
            if not screenGui or not screenGui.Parent then
                print("[Recovery] Recreating main GUI...")
                createGUI()
            end
            -- Cek floating logo (jika GUI tidak visible, logo harus ada)
